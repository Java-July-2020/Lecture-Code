package com.matthew.petswithafrontend22.controllers;
...
import com.matthew.petswithafrontend22.services.PetService;
import com.matthew.petswithafrontend22.services.TagService;
...
public class PetController {
	...
	@RequestMapping("/{id}")
	public String update(@PathVariable("id") Long id, @ModelAttribute("pet") Pet pet, Model model, @ModelAttribute("tag") Tag tag) {
		model.addAttribute("pet", pService.getOnePet(id));
		return "show.jsp";
	}
	
	@RequestMapping(value="/{id}", method=RequestMethod.PUT)
	public String updatePet(@Valid @ModelAttribute("pet") Pet pet, BindingResult result) {
		if(result.hasErrors()) {
	...
	public String createTag(@Valid @ModelAttribute("tag") Tag tag, BindingResult result, Model model) {
		...
		if(result.hasErrors()) {
			model.addAttribute("pet", this.pService.getOnePet(tag.getPet().getId()));
			return "show.jsp";
		} else {
